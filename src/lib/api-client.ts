...
// Fonction pour parser un fichier CSV nativement
export const parseCSV = (file: File): Promise<{ columns: string[], rows: number, data: any[] }> => {
  return new Promise((resolve, reject) => {
    ...
    // Vérifier d'abord si le fichier n'est pas vide
    if (file.size === 0) {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Parser les lignes en gérant les guillemets et les virgules
        ...
        // Parser la première ligne pour les colonnes
...
// Client WebSocket pour le streaming de logs
export class LogStreamClient {
  ...
  connect(recordId: number, onLog: (log: any) => void) {
    ...
    const wsUrl = `ws://localhost:8001/ws/logs/${recordId}`;
...
export const collectionApi = {
  uploadAndAnalyze: async (formData: FormData) => {
    const response = await apiClient.post("/collect-and-analyze", formData, {
      headers: {
